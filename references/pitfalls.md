...
16. **本地路径相对脚本而非 cwd**：部分脚本的输入路径相对脚本所在目录解析，调用时用
    绝对路径或先 cd。
17. **"这是什么"会被自信编造，主视觉准但上下文/小字/小图标不可靠**（实测：搜狗输入法
    皮肤截图 239×247，mimo 正确认出动漫少女，却定性为"游戏角色卡片"，把"中"字读成
    "白"、把衣服图标读成"点赞/菱形"、编造"夕"字角色名）。**解法**：凡是涉及"这是什么
    软件/页面/图标/小字"的结论，必须多 pass 交叉验证——全图中性描述 + OCR + detect 枚举
    + 按位置 glance --region 放大，数条证据一致才下结论；主视觉主体描述可单 pass 采信。
    小字/小图标读不准的根因是分辨率，已由**核心自动放大**修复：`VISION_MIN_UPSCALE=800`
    （默认）把最长边不足 800px 的小图 LANCZOS 放大到 800 再送，实测'中'字从读错变读对。
    自动放大对**大图无影响**（≥800px 原样发送）；只有"这是什么软件"的定性仍需交叉验证。

## 四、OCR 长截图

18. **分段边界丢字**：长截图不分段整张送会因下采样丢字。**解法**：long-ocr 分段 +
    重叠合并；文字极小先 crop --scale 放大。
19. **聊天记录气泡归属**：普通 OCR 会把多行气泡拼成一段。**解法**：long-ocr --mode
    chat，按气泡拆消息并保留昵称。
