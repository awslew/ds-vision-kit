...
def command_for_path(path: Path) -> list[str]:
    """Return the argv list that runs the glance command at *path*.

    Fixes a Windows pitfall: extensionless shebang Python scripts cannot be
    exec()'d directly (WinError 193), so they must be run through the current
    interpreter. On POSIX a non-executable script is also run through the
    interpreter.
    """
    ...
    if os.name == "nt":
        try:
            ...
            first_line = ""
        looks_like_python = first_line.startswith("#!") and "python" in first_line.lower()
        if looks_like_python or not path.suffix:
            # Extensionless (or python-shebang) file: run via the interpreter.
            return [sys.executable, str(path)]
        return [str(path)]
...
def resolve_glance_command() -> list[str]:
    """Locate glance: explicit override, then PATH (incl. .bat/.cmd/.exe on
    Windows), then this repository's bundled bin/glance."""
    override = os.environ.get("VISION_GLANCE_BIN", "").strip()
    if override:
    ...
    if not discovered and os.name == "nt":
        for ext in (".bat", ".cmd", ".exe"):
            discovered = shutil.which("glance" + ext)
            if discovered:
                break
    if discovered:
        return command_for_path(Path(discovered))
    # Bundled copy: this script lives in <repo>/scripts/, so the repo root is
    # parents[1]. Also try the older installed-skill layout (parents[3]).
    script_parents = Path(__file__).resolve().parents
    for idx in (1, 3):
        if idx < len(script_parents):
            repository_glance = script_parents[idx] / "bin" / "glance"
            if repository_glance.is_file():
                return [sys.executable, str(repository_glance)]
...
def run_glance(command: Sequence[str], timeout: float, chunk_index: int) -> str:
    # encoding must be explicit: on a Chinese Windows the locale is GBK, and
    # text=True then fails to decode the UTF-8 Chinese output of glance.
